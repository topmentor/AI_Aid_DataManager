...
/**
 * 데이터 소스 카탈로그 DAO — 중앙 app.db {@code ac_catalog}.
...
public class CatalogDAO {
    ...
    public static List<ResultMap> list() {
        try (Connection c = AppDb.conn()) {
            return SqliteUtil.queryForMapList(c,
                    "SELECT id, name, type, config, created_at FROM ac_catalog ORDER BY created_at", null);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static ResultMap get(String id) {
        try (Connection c = AppDb.conn()) {
            return SqliteUtil.queryForMap(c,
                    "SELECT id, name, type, config, created_at FROM ac_catalog WHERE id=?",
                    new Object[]{id});
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public static String add(String name, String type, String configJson) {
        ...
        try (Connection c = AppDb.conn()) {
            int n = SqliteUtil.update(c,
                    "INSERT INTO ac_catalog(id, name, type, config, created_at) VALUES(?,?,?,?,?)",
    ...
    public static int update(String id, String name, String type, String configJson) {
        try (Connection c = AppDb.conn()) {
            return SqliteUtil.update(c,
                    "UPDATE ac_catalog SET name=?, type=?, config=? WHERE id=?",
    ...
    public static int remove(String id) {
        try (Connection c = AppDb.conn()) {
            return SqliteUtil.update(c, "DELETE FROM ac_catalog WHERE id=?", new Object[]{id});
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
