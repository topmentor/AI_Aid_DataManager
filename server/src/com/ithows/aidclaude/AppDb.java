 ...
 *
 * <p>테이블: {@code ac_settings}, {@code ac_catalog}, {@code ac_jobs}.
...
public final class AppDb {
    ...
    public static Connection conn() throws SQLException {
        return SqliteUtil.open(AcContext.appDbFile().getAbsolutePath());
    }
    ...
    public static void init() throws SQLException {
        try (Connection c = conn()) {
            SqliteUtil.execScript(c,
              ...
              + "  value TEXT"
              + ");"
              + "CREATE TABLE IF NOT EXISTS ac_catalog ("
              + "  id TEXT PRIMARY KEY,"
              + "  name TEXT NOT NULL,"
              + "  type TEXT NOT NULL,"
              + "  config TEXT,"
              + "  created_at INTEGER"
              + ");"
              + "CREATE TABLE IF NOT EXISTS ac_jobs ("
              + "  id TEXT PRIMARY KEY,"
              + "  user_request TEXT,"
              + "  status TEXT,"
              + "  error_msg TEXT,"
